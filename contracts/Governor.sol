...
contract Governor is GovernorBravoDelegateStorageV2, GovernorBravoEvents {
    modifier onlyProfileOwner() {
        require(
            IERC721(_hub).ownerOf(_profileId) == msg.sender ||
                ILensHub(_hub).getDispatcher(_profileId) == msg.sender,
    ...
    uint256 internal immutable _profileId;
    address internal immutable _hub;
    address internal immutable _followNFT;

    constructor(uint256 profileId_, address hub_, uint256 timelockDelay) {
        _profileId = profileId_;
        _hub = hub_;

        _followNFT = ILensHub(_hub).getFollowNFT(profileId_);
        require(
            _followNFT != address(0),
            "Governor::initialize: invalid token address"
        );
        timelock = TimelockInterface(
            address(new Timelock(address(this), timelockDelay))
        );
    ...
    function propose(
        ...
        uint256 votingDelay,
        uint256 quorumVotes
    ) public onlyProfileOwner returns (uint256) {
        require(
        ...
        require(
            IFollowNFT(_followNFT).getDelegatedSupplyByBlockNumber(
                block.number
            ) <= quorumVotes,
            "not enough follower"
        );
     ...
     */
    function cancel(uint256 proposalId) external onlyProfileOwner {
        require(
    ...
    function state(uint256 proposalId) public view returns (ProposalState) {
        ...
        if (proposal.canceled) {
            ...
            proposal.quorumVotes > proposal.forVotes
        ) {
    ...
    function castVoteInternal(
        ...
        Proposal storage proposal = proposals[proposalId];
        Receipt storage receipt = _receipts[
            keccak256(abi.encodePacked(proposalId, voter))
        ];
        require(
            receipt.hasVoted == false,
            "Governor::castVoteInternal: voter already voted"
        );
        uint256 votes = IFollowNFT(_followNFT).getPowerByBlockNumber(
            voter,
            proposal.startBlock
        );
        if (support == 0) {
